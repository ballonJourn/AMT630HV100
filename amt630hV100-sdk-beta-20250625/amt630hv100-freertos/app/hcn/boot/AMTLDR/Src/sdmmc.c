...
void updateFromSD(int chipid)
{
	...
	SendUartString("load stepldr... \r\n");	
	fret = f_open(&fp, STEPLDR_FILE_NAME, FA_OPEN_EXISTING | FA_READ);
	if(fret != FR_OK)
	{
		SendUartString("Open file fail\r\n");
		return;
	}
	fret = f_read(&fp, (void *)STEPLDR_ENTRY, STEPLDR_MAX_SIZE, &size);
	if(fret != FR_OK)
	{
...
//	blkstart = offset;//offset/write_bl_len;
...
unsigned int mmc_read(void *buf, unsigned int offset, unsigned int size)
{
	...
	mmc_bread(blkstart, blkcnt, buf);

	return 0;
}

int EmmcInit(int chipid)
{
	lg_ulChip = chipid;
	g_MatchSdVcc = 0;
...
int EmmcReadSysInfo(SysInfo *info)
{
	...
	mmc_read(data, startoffset, sizeof(SysInfo));
...
{	
	...
	PrintVariableValueHex("stepldr_offset", pSysInfo->stepldr_offset);
    blksart = pSysInfo->stepldr_offset / 512;
	nSectorCount = DIV_ROUND_UP(STEPLDR_MAX_SIZE, SDMMC_SECTOR_SIZE);         
	if (mmc_read_blocks((UINT32 *)STEPLDR_ENTRY, blksart, nSectorCount) != nSectorCount)
	{
		SendUartString("EMMC Read fail.\r\n");
		//return;
	}

	SendUartString("Load STEP !!!\r\n");
	funPtr = (void (*)(void))STEPLDR_ENTRY;
	funPtr();
...
{	
	...
	PrintVariableValueHex("loader_offset", pSysInfo->loader_offset);
	nSectorCount = DIV_ROUND_UP(LOADER_MAX_SIZE, SDMMC_SECTOR_SIZE);
	if (mmc_read_blocks((UINT32 *)IMAGE_ENTRY, pSysInfo->loader_offset / 512, nSectorCount) != nSectorCount)
	{
		SendUartString("EMMC Read fail.\r\n");
		//return;
	}
