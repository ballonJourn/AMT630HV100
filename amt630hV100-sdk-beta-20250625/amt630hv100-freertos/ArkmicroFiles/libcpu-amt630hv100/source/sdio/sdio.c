...
#define SDIO_STACK_SIZE 			4096
//#define SDIO_THREAD_PRIORITY		15
#define SDIO_THREAD_PRIORITY		31

static List_t sdio_cards;
static List_t sdio_drivers;
                             ...
                             uint8_t            raw)
{
    ...
    configASSERT(card != NULL);
    configASSERT(fn <= SDIO_MAX_FUNCTIONS);
                               ...
                               uint32_t           blksize)
{
    ...
    configASSERT(card != NULL);
    configASSERT(fn <= SDIO_MAX_FUNCTIONS);
    configASSERT(blocks != 1 || blksize <= 512);
    configASSERT(blocks != 0);
    //configASSERT(blksize != 0);
    if (blksize == 0) {
		printf("\r\n %s blksize = 0 \r\n", __func__);
		return -1;
    }
...
static int32_t sdio_read_cccr(struct mmcsd_card *card)
{
    ...
    {
        TRACE_ERROR("unrecognised CCCR structure version %d", cccr_version);
...
static int32_t cistpl_funce_func0(struct mmcsd_card *card,
                                     const uint8_t     *buf,
                                     uint32_t           size)
{
...
static int32_t cistpl_funce_func(struct sdio_function *func,
                                    const uint8_t        *buf,
                                    uint32_t              size)
{
    uint32_t version;
...
static int32_t sdio_read_cis(struct sdio_function *func)
{
    ...
    do {
        ...
        curr = pvPortMalloc(sizeof(struct sdio_function_tuple) + tpl_link);
        if (!curr)
            return -ENOMEM;
        ...
        case CISTPL_MANFID:
            if (tpl_link < 4)
            {
                TRACE_DEBUG("bad CISTPL_MANFID length");
                break;
            }
            if (func->num != 0)
            {
        ...
        case CISTPL_FUNCE:
            ...
            if (ret)
            {
                TRACE_DEBUG("bad CISTPL_FUNCE size %u "
        ...
        case CISTPL_VERS_1:
            if (tpl_link < 2)
            {
                TRACE_DEBUG("CISTPL_VERS_1 too short");
            }
            break;
        default: 
            ...
            *prev = curr;
            prev = &curr->next;
            TRACE_DEBUG( "function %d, CIS tuple code %#x, length %d",
...
static int32_t sdio_initialize_function(struct mmcsd_card *card,
                                           uint32_t           func_num)
{
    int32_t ret;
    struct sdio_function *func;

    configASSERT(func_num <= SDIO_MAX_FUNCTIONS);

    func = pvPortMalloc(sizeof(struct sdio_function));
    if (!func)
    {
        TRACE_ERROR("malloc sdio_function failed");
        ret = -ENOMEM;
        goto err;
    }
	...
	if (func->max_blk_size > 0)
		sdio_set_block_size(func, func->max_blk_size);
	else
...
static int32_t sdio_register_card(struct mmcsd_card *card)
{
    ...
    sc = pvPortMalloc(sizeof(struct sdio_card));
    if (sc == NULL)
    {
        TRACE_ERROR("malloc sdio card failed");
        return -ENOMEM;
    }

    sc->card = card;
	vListInitialiseItem(&sc->list);
	listSET_LIST_ITEM_OWNER(&sc->list, sc);
    vListInsertEnd(&sdio_cards, &sc->list);

    if (listLIST_IS_EMPTY(&sdio_drivers))
    {
        goto out;
    }

	list_for_each_entry(pxListItem, sd, &sdio_drivers)
    {
...
static int32_t sdio_init_card(struct mmcsd_host *host, uint32_t ocr)
{
    ...
    card->sdio_function[0] = pvPortMalloc(sizeof(struct sdio_function));
    if (!card->sdio_function[0])
    {
        TRACE_ERROR("malloc sdio_func0 failed");
        err = -ENOMEM;
        goto err1;
    }
	...
	wifi_sdio_func = card->sdio_function[1];
    ...
    if (host->card)
    {
        for (i = 1; i < host->card->sdio_function_num + 1; i++)
        {
            if (host->card->sdio_function[i])
            {
                ...
                vPortFree(host->card->sdio_function[i]);
...
err2:
    if (host->card && host->card->sdio_function[0])
    {
        ...
        vPortFree(host->card->sdio_function[0]);
...
err:
    TRACE_ERROR("error %d while initialising SDIO card", err);
...
int32_t init_sdio(struct mmcsd_host *host, uint32_t ocr)
{
    ...
    if (ocr & 0x7F) 
    {
        TRACE_WARNING("Card ocr below the defined voltage rang.\r\n");
        ocr &= ~0x7F;
    }

    if (ocr & VDD_165_195) 
    {
        TRACE_WARNING("Can't support the low voltage SDIO card.\r\n");
        ocr &= ~VDD_165_195;
    }
...
err:
    ...
    TRACE_ERROR("init SDIO card failed");
...
static void sdio_irq_thread(void *param)
{
    ...
    while (1) 
    {
        if (xSemaphoreTake(host->sdio_irq_sem, portMAX_DELAY) == pdPASS)
        {
...
static int32_t sdio_irq_thread_create(struct mmcsd_card *card)
{
    ...
    {
        host->sdio_irq_num++;
        host->sdio_irq_sem = xSemaphoreCreateCounting(32, 0);
        configASSERT(host->sdio_irq_sem != NULL);

        xTaskCreate(sdio_irq_thread, "sdio_irq", SDIO_STACK_SIZE, 
        	host, SDIO_THREAD_PRIORITY, &host->sdio_irq_thread);//
    }
...
static int32_t sdio_irq_thread_delete(struct mmcsd_card *card)
{
    ...
    configASSERT(host->sdio_irq_num > 0);
    ...
    if (!host->sdio_irq_num) 
    {
        ...
        vSemaphoreDelete(host->sdio_irq_sem);
        host->sdio_irq_sem = NULL;
        vTaskDelete(host->sdio_irq_thread);
...
void sdio_irq_wakeup_isr(struct mmcsd_host *host)
{
    ...
    if (host->sdio_irq_sem)
        xSemaphoreGiveFromISR(host->sdio_irq_sem, 0);
}
...
void sdio_irq_wakeup(struct mmcsd_host *host)
{
    if (host->flags & MMCSD_SUP_SDIO_IRQ)
        host->ops->enable_sdio_irq(host, 0);
    if (host->sdio_irq_sem)
        xSemaphoreGive(host->sdio_irq_sem);
}
...
int32_t sdio_enable_func(struct sdio_function *func)
{
    ...
    timeout = xTaskGetTickCount() + func->enable_timeout_val * configTICK_RATE_HZ / 1000;
...
static struct mmcsd_card *sdio_match_driver(struct sdio_device_id *id)
{
	...
	list_for_each_entry(pxListItem, sc, &sdio_cards)
    {
        card = sc->card;
...
int32_t sdio_register_driver(struct sdio_driver_t *driver)
{
    ...
    sd = pvPortMalloc(sizeof(struct sdio_driver));
    if (sd == NULL)
    {
        TRACE_ERROR("malloc sdio driver failed");

        return -ENOMEM;
    }

    sd->drv = driver;
	vListInitialiseItem(&sd->list);
	listSET_LIST_ITEM_OWNER(&sd->list, sd);
	vListInsertEnd(&sdio_drivers, &sd->list);

    if (!listLIST_IS_EMPTY(&sdio_cards))
    {
...
int32_t sdio_unregister_driver(struct sdio_driver_t *driver)
{
    ...
    if (sd == NULL)
    {
        TRACE_ERROR("SDIO driver %s not register", driver->name);
        return -1;
    }
...
void sdio_init(void)
{
	vListInitialise(&sdio_cards);
	vListInitialise(&sdio_drivers);
}
