...
/* static void usb_disk_file_test()
{
	char buf[128] = {0};
	int ret, i;
	FF_FILE *fp =  ff_fopen("/usb/zImage", "rb");
	if (NULL == fp) {
		printf("fopen /usb/zImage failed\r\n");
		return;
	}
	ret = ff_fread(buf, 1, 128, fp);
	if (ret > 0) {
		for (i = 0; i < ret; i++) {
			if (i % 16 == 0)
				printf("\r\n");
...
{
	if (0 == usb_media_ready)
		return;
	if (usb_max_devs > 0) {
		usb_send_stor_dev_pluged(0);
	}
...
void usb_stor_disconnect_isr()
{
	usb_media_ready = 0;
	if (usb_max_devs > 0) {
		usb_send_stor_dev_pluged_isr(0);
	}
}
