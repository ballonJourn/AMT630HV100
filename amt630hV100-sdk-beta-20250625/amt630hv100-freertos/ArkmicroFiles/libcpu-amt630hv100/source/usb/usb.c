...
#define USB_WAIT_DEV_TIMEOUT	30000
#define USB_MOUNT_PATH	"/usb"
extern int	hub_status_data(char buf[8]);
void usb_stor_disconnect();
static char gconnect_flag = 0;
static FF_Disk_t *usb_disk = NULL;

static int usb_mode;	//0:usb_host 1:usb_device
static char connect_status = 0;
static TaskHandle_t	scan_usb_dev_task;
static TimerHandle_t usbPortScanTimer;
static QueueHandle_t 	wait_new_dev;
static QueueHandle_t	usb_stor_dev_pluged;
static SemaphoreHandle_t	dev_lock;

#define mainTIMER_SCAN_FREQUENCY_MS			pdMS_TO_TICKS( 1000UL )


#ifndef SendUartString
#define SendUartString printf
#endif
 ...
 */
static void usb_set_maxpacket_ep(struct usb_device *dev, int if_idx, int alt_idx, int ep_idx)
{
	int b;
	struct usb_endpoint_descriptor *ep;
	u16 ep_wMaxPacketSize;
	unsigned char *tmp;
	struct usb_interface *if_desc = &dev->config.if_desc[if_idx];

	if (if_desc->num_altsetting <= 1)
		ep = &dev->config.if_desc[if_idx].ep_desc[ep_idx];
	else if (if_desc->num_altsetting > 1) {
		ep = &if_desc->alt_intf[alt_idx].ep_desc[ep_idx];
	}
...
{
	usb_set_maxpacket_ep(dev, if_idx, alt_idx, ep_idx);
}
...
static int usb_set_maxpacket(struct usb_device *dev)
{
	...
	for (i = 0; i < dev->config.desc.bNumInterfaces; i++) {
		//struct usb_interface *if_desc = &dev->config.if_desc[i];
		struct usb_alt_interface *alt_if = &dev->config.if_desc[i].alt_intf[0];
		//for (ii = 0; ii < dev->config.if_desc[i].desc.bNumEndpoints; ii++)
		//	usb_set_maxpacket_ep(dev, i, ii);
		for (ii = 0; ii < alt_if->desc.bNumEndpoints; ii++) {
			usb_set_maxpacket_ep(dev, i, 0, ii);
		}
			...
			void *data, int len, int *actual_length, int timeout)
{
	...
	if (((int)data) % ARCH_DMA_MINALIGN == 0) {
		err = submit_bulk_msg(dev, pipe, data, len, &act_len, timeout);
		if (dev->status || (err < 0))
			return -1;
...
int usb_iso_msg(struct usb_device *dev, unsigned int pipe,
			void *data, int len, int *actual_length, int timeout)
{
	return submit_iso_msg(dev, pipe, data, len, actual_length, timeout);
}
			...
			unsigned char *buffer, int cfgno)
{
	...
	int index, ifno, epno, curr_if_num, cur_alt_if_num, alt_if_ep_idx;
	u16 ep_wMaxPacketSize;
	unsigned char *tmp;
	struct usb_interface *if_desc = NULL;
	struct usb_alt_interface *alt_if = NULL;
	...
	while (index + 1 < dev->config.desc.wTotalLength && head->bLength) {
		...
		case USB_DT_INTERFACE:
			...
			if (((struct usb_interface_descriptor *) \
				...
				alt_if = &if_desc->alt_intf[0];
				memcpy(alt_if, head, USB_DT_INTERFACE_SIZE);
				curr_if_num =
				...
				if (ifno >= 0) {
					if_desc = &dev->config.if_desc[ifno];
					alt_if = &if_desc->alt_intf[if_desc->num_altsetting];
					memcpy(alt_if, head, USB_DT_INTERFACE_SIZE);
					cur_alt_if_num = if_desc->num_altsetting;
					alt_if_ep_idx = 0;
		...
		case USB_DT_ENDPOINT:
			...
			USB_UNUSED(ep_wMaxPacketSize);
			if (if_desc->num_altsetting > 1) {
				memcpy(&if_desc->alt_intf[cur_alt_if_num].ep_desc[alt_if_ep_idx++], head,
					USB_DT_ENDPOINT_SIZE);
				if_desc->alt_intf[cur_alt_if_num].ep_desc[alt_if_ep_idx++].wMaxPacketSize = ep_wMaxPacketSize;
			}
...
int usb_set_interface(struct usb_device *dev, int interface, int alternate)
{
	...
	if (if_face->num_altsetting > 1) {
		int i;
		struct usb_alt_interface *alt_if = &if_face->alt_intf[alternate];
		for (i = 0; i < alt_if->desc.bNumEndpoints; i++) {
			usb_set_maxpacket_ep(dev, interface, alternate, i);
		}
	}
			      ...
			      struct usb_device *parent)
{
	...
	dev->devnum = addr;printf("hub_set_address:4294\r\n");
...
static void usb_hub_power_on(struct usb_hub_device *hub)
{
	 ...
	 */
	hub->query_delay = xTaskGetTickCount() + pdMS_TO_TICKS(max(100, (int)pgood_delay));
	 ...
	 */
	hub->connect_timeout = hub->query_delay + pdMS_TO_TICKS(1000);
...
#ifdef USB_TEST_EYE_DIAGRAM
		if (1) {
			usb_set_port_feature(dev, (4 << 8), 21);
			while(1)
				vTaskDelay(100);
		}
#endif
...
static int usb_scan_port(struct usb_device_scan *usb_scan)
{
	 ...
	 */
	if (xTaskGetTickCount() < hub->query_delay)
		return 0;
	...
	if (ret < 0) {
		//debug("get_port_status failed\n");
		if (xTaskGetTickCount() >= hub->connect_timeout) {
	...
	if (ret < 0) {
		printf("usb port change err at line:%d\r\n", __LINE__);
		goto exit;
	}
...
exit:
	...
	free(usb_scan);printf("usb_scan_port  ret:%d\r\n", ret);
...
static int usb_hub_configure(struct usb_device *dev)
{
	...
	if (hub->pusb_dev && hub->pusb_dev->parent) {// not root hub
        printf("usb hub found, but not root\r\n");
		//for (i = 0; i < dev->maxchild; i++) {
		for (i = 0; i < 1; i++) {
			struct usb_device_scan *usb_scan;
		...
		xQueueSend(wait_new_dev, NULL, 0);
	}
...
extern int dwc2_disconnect_flag;
static void notify_new_dev_attach()
{
	...
	dwc2_disconnect_flag = 0;
	xQueueSend(wait_new_dev, NULL, 0);
}

static void usb_dev_scan_proc(void *pvParameters)
{
	//struct usb_hub_device *hub;
	...
	//hub = usb_get_hub_device(dev);
	while(usb_started) {
		ret = xQueueReceive(wait_new_dev, NULL, portMAX_DELAY);
		if (pdFALSE == ret) {
			continue;
		}
		xTimerStop(usbPortScanTimer, 0);
		//xSemaphoreTake(list_lock, portMAX_DELAY);
		usb_disable_endpoint();
		ret = usb_device_list_scan();
		//xSemaphoreGive(list_lock);
		xTimerStart( usbPortScanTimer, 0 );
	}
	vTaskDelete(NULL);
}

void hub_usb_dev_reset(void)
{
	int i;
	struct usb_device *dev;
	struct usb_hub_device* hub;
	...
	for (i = 1; i < USB_MAX_HUB; i++) {
		hub = &hub_dev[i];
		if (hub->pusb_dev != NULL) {printf("free hub\r\n");
			hub->pusb_dev = NULL;
			if (usb_hub_index > 1)
				usb_hub_index--;
		}
...
static void prvUsbPortScanTimerCallback( TimerHandle_t xTimerHandle )
{
	char buf[8] = {0};
	int ret = 1;
	char status = 0;
	...
	if (connect_status != status) {
		printf("state:%d ret:%d usb state changed now!\r\n", buf[0], ret);
...
int usb_send_stor_dev_pluged(uint8_t pluged)
{
	if (xQueueSend(usb_stor_dev_pluged, &pluged, 0) != pdPASS)
		return -1;
	return 0;
}

int usb_send_stor_dev_pluged_isr(uint8_t pluged)
{
	if (pluged == 0)
		gconnect_flag = 0;
	if (xQueueSendFromISR(usb_stor_dev_pluged, &pluged, 0) != pdPASS)
		return -1;
	return 0;
}


int usb_wait_stor_dev_pluged(uint32_t timeout)
{
    uint8_t pluged = 0;
	uint32_t stick = xTaskGetTickCount();

	while (!usb_stor_dev_pluged) {
		if (xTaskGetTickCount() - stick > timeout)
			return -1;
		vTaskDelay(10);
	}

    if (usb_stor_dev_pluged && xQueueReceive(usb_stor_dev_pluged, &pluged, timeout) == pdPASS)
    {
        if(pluged)
        {
        	usb_disk = FF_USBDiskInit(USB_MOUNT_PATH);
			if (usb_disk)
            	return USB_DEV_PLUGED;
			else
				return USB_DEV_UNPLUGED;
        }
        else
        {
        	if (usb_disk) {
				FF_USBDiskDelete(usb_disk);
				usb_disk = NULL;
			}
            return USB_DEV_UNPLUGED;
        }
...
int g_zero_register(const char *name);
...
int ark_usb_init()
{
	...
	vSysctlConfigure(SYS_PAD_CTRL06, 0, 0xff, 0);
	gpio_request(USB_MODE_ID);
	gpio_direction_input(USB_MODE_ID);
	usb_mode = gpio_get_value(USB_MODE_ID);
#else
	usb_mode = 0;
#endif

	wait_new_dev = xQueueCreate(1, 0);
	usb_stor_dev_pluged = xQueueCreate(1, 1);
	dev_lock = xSemaphoreCreateMutex();
	//USB_UNUSED(list_lock);
#ifdef USB_UVC_SUPPORT
	usb_uvc_init();
#endif
	usb_sysctrl_init();
	...
	//g_ncm_register("ncm");
		...
		usb_started = 1;
			...
			if (0) {
				...
				//usb_hub_port_reset(dev, port, &portstatus);
				usb_set_port_feature(dev, (4 << 8), 21);
				while(1)
					vTaskDelay(100);
			}
		...
		usbPortScanTimer = xTimerCreate( "Timer",
									mainTIMER_SCAN_FREQUENCY_MS,		/* The period of the software timer in ticks. */
									pdTRUE,			/* xAutoReload is set to pdFALSE, so this is a one-shot timer. */
									NULL,				/* The timer's ID is not used. */
									prvUsbPortScanTimerCallback );
		xTaskCreate(usb_dev_scan_proc, "usb_scan", configMINIMAL_STACK_SIZE * 3, (void*)dev, configMAX_PRIORITIES, &scan_usb_dev_task);
