...
usb_copy_descriptors(struct usb_descriptor_header **src)
{
	...
	ret = mem;
       mem_ptr = (uint8_t *)mem;
       mem_ptr += (n_desc + 1) * sizeof(*tmp);
	mem = (void*)mem_ptr;
	while (*src) {
		memcpy(mem, (void *)*src, (*src)->bLength);
		*tmp = mem;
		tmp++;
              mem_ptr = (uint8_t *)mem;
		mem_ptr += (*src)->bLength;
		...
		enum usb_device_speed speed, void *buf, u8 type)
{
	...
	void				*next = (void *)((uint8_t*)buf + USB_DT_CONFIG_SIZE);
	...
	if (config->descriptors) {
		...
		if (status < 0)
			return status;
		len -= status;
             next_ptr = (uint8_t*)next;
		next_ptr += status;
             next  = (void*)next_ptr;
	}

	/* add each function's descriptors */
	list_for_each_entry(pxListItem, f, &config->functions) {
...
static int config_desc(struct usb_composite_dev *cdev, unsigned w_value)
{
	...
	w_value &= 0xff;
	list_for_each_entry(pxListItem, c, &cdev->configs) {
		if (speed == USB_SPEED_HIGH) {
			if (!c->highspeed)
				continue;
		} else {
...
static void reset_config(struct usb_composite_dev *cdev)
{
	...
	list_for_each_entry(pxListItem, f, &cdev->config->functions) {
		if (f->disable)
			f->disable(f);
		...
		void *buf, u16 language, int id)
{
	...
	if (id == 0) {
		...
		list_for_each_entry(pxListItem, c, &cdev->configs) {
			...
			list_for_each_entry(pxListItem1, f, &c->functions) {
				sp = f->strings;
				if (sp)
					collect_langs(sp, s->wData);
			}
...
composite_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *ctrl)
{
	...
	struct usb_configuration	*c;
       ListItem_t                   *pxListItem = NULL;
/*printf("+++%s:%d ctrl->bRequest = 0x%02x bRequestType = 0x%02x wValue = 0x%08x  wIndex= 0x%08x  wLength = 0x%08x +++\r\n",
		__func__, __LINE__, ctrl->bRequest,
		ctrl->bRequestType,  ctrl->wValue, ctrl->wIndex, ctrl->wLength);*/
	/*
...
unknown:
		...
		case USB_RECIP_ENDPOINT:
			...
			if (&f->list == listGET_HEAD_ENTRY(&cdev->config->functions))
				f = NULL;
			break;
		/*
...
static int composite_bind(struct usb_gadget *gadget)
{
	...
	cdev = (struct usb_composite_dev	*)kzalloc(sizeof *cdev, 0);
	if (!cdev)
		return status;
	...
	if (!cdev->req)
		goto fail;
	cdev->req->buf = kmalloc(USB_BUFSIZ, 0);
	if (!cdev->req->buf)
		goto fail;
