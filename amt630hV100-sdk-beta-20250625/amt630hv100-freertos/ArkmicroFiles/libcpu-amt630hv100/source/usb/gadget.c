...
static List_t                   free_request_list;
static SemaphoreHandle_t  free_list_lock;
static struct dwc2_hsotg_req *free_req;
//static void dwc2_hsotg_dump1(struct dwc2_hsotg *hsotg);
void dwc2_dump_global_registers1(struct dwc2_hsotg *hsotg);

static struct dwc2_hsotg_req* get_free_req()
{
    struct dwc2_hsotg_req* req = NULL;
    if (!xPortIsInInterrupt())
        xSemaphoreTake(free_list_lock, portMAX_DELAY);

    if (listLIST_IS_EMPTY(&free_request_list)) {
        printf("free list req is empty\r\n");
        goto exit;
    }
    req = listGET_OWNER_OF_HEAD_ENTRY(&free_request_list);
    uxListRemove(&req->free_list_entry);

exit:
    if (!xPortIsInInterrupt())
        xSemaphoreGive(free_list_lock);
    return req;
}

static void free_used_req(struct dwc2_hsotg_req* req)
{
    if (NULL == req)
        return;
    ...
    vListInsertEnd(&free_request_list, &req->free_list_entry);
...
static inline struct dwc2_hsotg_req *our_req(struct usb_request *req)
{
	return (struct dwc2_hsotg_req *)req->powner;
}

static inline struct dwc2_hsotg_ep *our_ep(struct usb_ep *ep)
{
	return (struct dwc2_hsotg_ep *)ep->powner;
}

static inline struct dwc2_hsotg *to_hsotg(struct usb_gadget *gadget)
{
	return (struct dwc2_hsotg *)gadget->powner;
}

static inline void __orr32(u32 ptr, u32 val)
{
	dwc2_writel(dwc2_readl(ptr) | val, ptr);
}

static inline void __bic32(u32 ptr, u32 val)
{
	dwc2_writel(dwc2_readl(ptr) & ~val, ptr);
}
#endif
						       ...
						       gfp_t flags)
{
	...
	req = get_free_req();//kzalloc(sizeof(*req), flags);
	if (!req)
		return NULL;
#ifndef NO_GNU
	INIT_LIST_HEAD(&req->queue);
#else
	INIT_LIST_ITEM(&req->queue);
	listSET_LIST_ITEM_OWNER(&req->queue, req);
	req->req.powner = req;
#endif
...
static int dwc2_gadget_alloc_ctrl_desc_chains(struct dwc2_hsotg *hsotg)
{
	...
	if (!hsotg->ctrl_out_desc)
		goto fail;
#ifdef NO_GNU
	if ((u32)hsotg->setup_desc[0] & (ARCH_DMA_MINALIGN - 1)) {
		u32 addr = (u32)(((u32)hsotg->setup_desc[0] + ARCH_DMA_MINALIGN) & (~(ARCH_DMA_MINALIGN - 1)));
		hsotg->setup_desc_dma_offset[0] = addr - (u32)hsotg->setup_desc[0];
		hsotg->setup_desc[0]		    = (struct dwc2_dma_desc *)addr;
		hsotg->setup_desc_dma[0]	    += hsotg->setup_desc_dma_offset[0];
	} else {
		hsotg->setup_desc_dma_offset[0] = 0;
	}

	if ((u32)hsotg->setup_desc[1] & (ARCH_DMA_MINALIGN - 1)) {
		u32 addr = (u32)(((u32)hsotg->setup_desc[1] + ARCH_DMA_MINALIGN) & (~(ARCH_DMA_MINALIGN - 1)));
		hsotg->setup_desc_dma_offset[1] = addr - (u32)hsotg->setup_desc[0];
		hsotg->setup_desc[1]		    = (struct dwc2_dma_desc *)addr;
		hsotg->setup_desc_dma[1]	    += hsotg->setup_desc_dma_offset[1];
	} else {
		hsotg->setup_desc_dma_offset[1] = 0;
	}

	if ((u32)hsotg->ctrl_in_desc & (ARCH_DMA_MINALIGN - 1)) {
		u32 addr = (u32)(((u32)hsotg->ctrl_in_desc + ARCH_DMA_MINALIGN) & (~(ARCH_DMA_MINALIGN - 1)));
		hsotg->ctrl_in_desc_offset  = addr - (u32)hsotg->ctrl_in_desc;
		hsotg->ctrl_in_desc 		= (struct dwc2_dma_desc *)addr;
		hsotg->ctrl_in_desc_dma     += hsotg->ctrl_in_desc_offset;
	} else {
		hsotg->ctrl_in_desc_offset = 0;
	}

	if ((u32)hsotg->ctrl_out_desc & (ARCH_DMA_MINALIGN - 1)) {
		u32 addr = (u32)(((u32)hsotg->ctrl_in_desc + ARCH_DMA_MINALIGN) & (~(ARCH_DMA_MINALIGN - 1)));
		hsotg->ctrl_out_desc_offset  = addr - (u32)hsotg->ctrl_out_desc;
		hsotg->ctrl_out_desc 		 = (struct dwc2_dma_desc *)addr;
		hsotg->ctrl_out_desc_dma     += hsotg->ctrl_out_desc_offset;
	} else {
		hsotg->ctrl_out_desc_offset = 0;
	}
				...
				struct dwc2_hsotg_req *hs_req)
{
	...
	data = (u32)(hs_req->req.buf) + buf_pos;

	iowrite32_rep(hsotg->regs + EPFIFO(hs_ep->index), (const void*)data, to_write);
...
static void dwc2_gadget_start_isoc_ddma(struct dwc2_hsotg_ep *hs_ep)
{
    ...
    list_for_each_entry_safe(pxListItem, nListItem, hs_req, &hs_ep->queue) {
#endif
				...
				bool continuing)
{
	...
	if (using_desc_dma(hsotg)) {
		...
		if (using_dma(hsotg) && !continuing && (length != 0)) {
			 ...
			 */
#ifdef NO_GNU
		       CP15_flush_dcache_for_dma((uint32_t)ureq->dma, (uint32_t)((uint32_t)ureq->dma + length));
#endif
			dwc2_writel(ureq->dma, hsotg->regs + dma_reg);

			dev_dbg(hsotg->dev, "%s: %pad => 0x%08x epsize:%d length:%d 0x%08x 0x%08x\r\n",
				__func__, &ureq->dma, dma_reg, epsize, length, ureq->buf, ureq->dma);
		}
	}
...
#else
	INIT_LIST_ITEM(&hs_req->queue);
	hs_req->queue.pvOwner = (void *)hs_req;
#endif
	req->actual = 0;
				       ...
				       struct usb_request *req)
{
	...
	free_used_req(hs_req);//kfree(hs_req);
}
 ...
 */
/*#ifndef NO_GNU
#ifndef list_first_entry_or_null(ptr, type, member)
...
static struct dwc2_hsotg_req *get_ep_head(struct dwc2_hsotg_ep *hs_ep)
{
	...
	return list_first_entry_or_null(&hs_ep->queue, struct dwc2_hsotg_req,
					queue);
#else
	return list_first_entry(&hs_ep->queue);
#endif
}
				       ...
				       struct usb_ctrlrequest *ctrl)
{
	...
	if (ret == 0 && hsotg->driver) {
		...
		} else if (ret == 0 && ctrl->bRequest == USB_REQ_SET_INTERFACE
			&& hsotg->enumtimer_start) {
			xTimerStopFromISR(hsotg->enumtimer, 0);
			hsotg->enumtimer_start = 0;
		}
	}
...
#else
	if (!list_item_empty(&hs_req->queue)) {
		dev_dbg(hsotg->dev, "%s already queued???\n", __func__);
		return;
	}
#endif
...
static void dwc2_hsotg_rx_data(struct dwc2_hsotg *hsotg, int ep_idx, int size)
{
...
#else
	u32 fifo = hsotg->regs + EPFIFO(ep_idx);
#endif
	int to_read;
	int max_req;
	int read_ptr;
	...
	dev_dbg(hsotg->dev, "%s ep%d: read %d/%d, done %d/%d\r\n",
		__func__, ep_idx, to_read, max_req, read_ptr, hs_req->req.length);
	 ...
	 */
	ioread32_rep(fifo, (void*)((u32)hs_req->req.buf + read_ptr), to_read);
}
 ...
 */
#include "sysctl.h"
static void prvEnumTimerCallback( TimerHandle_t xTimerHandle )
{
	struct dwc2_hsotg *hsotg = (struct dwc2_hsotg *)pvTimerGetTimerID(xTimerHandle);

	TRACE_INFO("prvEnumTimerCallback\r\n");
...
static void dwc2_hsotg_irq_enumdone(struct dwc2_hsotg *hsotg)
{
	...
	case DSTS_ENUMSPD_FS48:
		...
		ep_mps = 1023;
		if (!hsotg->enumtimer_start) {
			xTimerStartFromISR(hsotg->enumtimer, 0);
			hsotg->enumtimer_start = 1;
		}
		break;
...
#else
	ListItem_t *pxListItem, *nListItem;
    list_for_each_entry_safe(pxListItem, nListItem, req, &ep->queue) {
...
irq_retry:
	...
	if (gintsts & (GINTSTS_OEPINT | GINTSTS_IEPINT)) {
		...
		dev_dbg(hsotg->dev, "#%s: daint=%08x daintmsk=%08x\r\n", __func__, daint, daintmsk);
...
#ifdef NO_GNU
		if ((u32)hs_ep->desc_list & (ARCH_DMA_MINALIGN - 1)) {
			u32 addr = (u32)(((u32)hs_ep->desc_list + ARCH_DMA_MINALIGN) & (~(ARCH_DMA_MINALIGN - 1)));
			hs_ep->desc_list_offset = addr - (u32)hs_ep->desc_list;
			hs_ep->desc_list 		= (struct dwc2_dma_desc	*)addr;
			hs_ep->desc_list_dma 	+= hs_ep->desc_list_offset;
		} else {
			hs_ep->desc_list_offset = 0;
		}
#endif
	}
...
#ifdef NO_GNU
		u32  addr = (u32)(hs_ep->desc_list) - hs_ep->desc_list_offset;
		hs_ep->desc_list 		= (struct dwc2_dma_desc	*)addr;
		hs_ep->desc_list_dma 	-= hs_ep->desc_list_offset;
#endif
		dmam_free_coherent(hsotg->dev, MAX_DMA_DESC_NUM_GENERIC *
			sizeof(struct dwc2_dma_desc),
			(void *)hs_ep->desc_list, hs_ep->desc_list_dma);
...
#else
	INIT_LIST_ITEM(&hs_ep->ep.ep_list);
	listSET_LIST_ITEM_OWNER(&hs_ep->ep.ep_list, &(hs_ep->ep));
#endif
...
static int dwc2_hsotg_hw_cfg(struct dwc2_hsotg *hsotg)
{
	...
	hsotg->eps_in[0] = (struct dwc2_hsotg_ep *)devm_kzalloc(hsotg->dev,
	...
	hsotg->eps_out[0] = hsotg->eps_in[0];
       hsotg->eps_in[0]->ep.powner = (void*)(hsotg->eps_in[0]);
       hsotg->eps_out[0]->ep.powner = (void*)(hsotg->eps_out[0]);
       listSET_LIST_ITEM_OWNER(&hsotg->eps_out[0]->ep.ep_list, &hsotg->eps_out[0]->ep);
       vListInitialise(&(hsotg->eps_out[0]->queue));
       listSET_LIST_ITEM_OWNER(&hsotg->eps_in[0]->ep.ep_list, &hsotg->eps_in[0]->ep);
       vListInitialise(&(hsotg->eps_in[0]->queue));
	...
	for (i = 1, cfg >>= 2; i < hsotg->num_of_eps; i++, cfg >>= 2) {
		...
		if (!(ep_type & 2)) {
			hsotg->eps_in[i] = (struct dwc2_hsotg_ep *)devm_kzalloc(hsotg->dev,
				sizeof(struct dwc2_hsotg_ep), GFP_KERNEL);
			if (!hsotg->eps_in[i])
				return -ENOMEM;
#ifdef NO_GNU
			hsotg->eps_in[i]->ep.powner = (void*)(hsotg->eps_in[i]);
                    listSET_LIST_ITEM_OWNER(&hsotg->eps_in[i]->ep.ep_list, &hsotg->eps_in[i]->ep);
                    vListInitialise(&(hsotg->eps_in[i]->queue));
#endif
		}
		/* Direction out or both */
		if (!(ep_type & 1)) {
			hsotg->eps_out[i] = (struct dwc2_hsotg_ep *)devm_kzalloc(hsotg->dev,
				sizeof(struct dwc2_hsotg_ep), GFP_KERNEL);
			if (!hsotg->eps_out[i])
				return -ENOMEM;
#ifdef NO_GNU
			hsotg->eps_out[i]->ep.powner = (void*)(hsotg->eps_out[i]);
                    listSET_LIST_ITEM_OWNER(&hsotg->eps_out[i]->ep.ep_list, &hsotg->eps_out[i]->ep);
                    vListInitialise(&(hsotg->eps_out[i]->queue));
#endif
		}
	}
 ...
 */
void dwc2_hsotg_init_wq_msg(struct dwc2_hsotg *hsotg);
int dwc2_gadget_init(struct dwc2_hsotg *hsotg, int irq)
{
...
#ifdef NO_GNU
	hsotg->gadget.powner = (void*)hsotg;
#endif
	...
	hsotg->ctrl_buff = (void*)devm_kzalloc(hsotg->dev,
			DWC2_CTRL_BUFF_SIZE, GFP_KERNEL);
	if (!hsotg->ctrl_buff)
		return -ENOMEM;

	hsotg->ep0_buff = (void*)devm_kzalloc(hsotg->dev,
			DWC2_CTRL_BUFF_SIZE, GFP_KERNEL);
	if (!hsotg->ep0_buff)
		return -ENOMEM;
	...
	hsotg->gadget.ep0 = &hsotg->eps_out[0]->ep;
       vListInitialise(&free_request_list);
       free_list_lock = xSemaphoreCreateMutex();
       free_req = (struct dwc2_hsotg_req*)kzalloc(sizeof(struct dwc2_hsotg_req) * 64, 0);
       xSemaphoreTake(free_list_lock, portMAX_DELAY);
       for (i = 0; i < 64; i++) {
            vListInitialiseItem(&free_req[i].free_list_entry);
            vListInsertEnd(&free_request_list, &free_req[i].free_list_entry);
            listSET_LIST_ITEM_OWNER(&free_req[i].free_list_entry,  &free_req[i]);
       }
       xSemaphoreGive(free_list_lock);
	...
	for (epnum = 0; epnum < hsotg->num_of_eps; epnum++) {//5
		if (hsotg->eps_in[epnum] && (epnum % 2))
			dwc2_hsotg_initep(hsotg, hsotg->eps_in[epnum],
					  epnum, 1);
		if (hsotg->eps_out[epnum] && (!(epnum % 2)))
	...
	hsotg->enumtimer = xTimerCreate( "EnumTimer",
						pdMS_TO_TICKS(1500), /* The period of the software timer in ticks. */
						pdFALSE,			/* xAutoReload is set to pdFALSE, so this is a one-shot timer. */
						(void*)hsotg,				/* The timer's ID is not used. */
						prvEnumTimerCallback );
