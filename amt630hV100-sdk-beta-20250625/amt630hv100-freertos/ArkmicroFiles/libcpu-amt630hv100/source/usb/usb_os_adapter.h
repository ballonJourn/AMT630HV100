...
#if USB_MODE_ID && USB_MODE_ID != -1	//usb device
...
#define spin_lock_irqsave(lock, flags)      \
do {                                        \
	(void)lock; flags = __get_interrupt_state(); portENTER_CRITICAL(); \
} while (0)

#define spin_unlock_irqrestore(lock, flags) \
do {                                        \
	(void)lock; portEXIT_CRITICAL();__set_interrupt_state(flags);			\
} while (0)
#endif
