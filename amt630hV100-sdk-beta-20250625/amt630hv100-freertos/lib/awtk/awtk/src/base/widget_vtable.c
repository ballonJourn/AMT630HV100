...
ret_t widget_paint_with_clip_dirty_rects(widget_t* widget, dirty_rects_t* clip, canvas_t* c,
	widget_on_paint_t on_paint) {
	dirty_rects_t r = { 0 };
	dirty_rects_t r_dirtys = { 0 };
	rect_t r_save = { 0 };
	rect_t r_vg_save = { 0 };
	vgcanvas_t* vg = canvas_get_vgcanvas(c);
	return_value_if_fail(widget != NULL && on_paint != NULL, RET_BAD_PARAMS);
	...
	if (clip != NULL) {
		//r = *clip;
		memcpy (&r, clip, sizeof(dirty_rects_t));
	}
	else {
		dirty_rects_init(&r);
		rect_t r_c = rect_init(c->ox, c->oy, widget->w, widget->h);
		dirty_rects_add(&r, &r_c);
	}
	rect_intersect_dirty_rects(&r, &r_save, &r_dirtys);
	canvas_set_clip_rect_dirty_rects (c, &r_dirtys);
	if (vg != NULL) {
		vgcanvas_clip_dirty_rects(vg, &r_dirtys);
	}
