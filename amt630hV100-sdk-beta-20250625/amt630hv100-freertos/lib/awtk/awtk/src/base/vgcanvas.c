...
ret_t vgcanvas_clip_dirty_rects(vgcanvas_t* vg, const dirty_rects_t* dirty_rects) {
	return_value_if_fail(vg != NULL && vg->vt->clip_dirty_rects != NULL, RET_BAD_PARAMS);
	float x, y, w, h;
	x = dirty_rects->max.x;
	y = dirty_rects->max.y;
	w = dirty_rects->max.w;
	...
	return vg->vt->clip_dirty_rects(vg, dirty_rects);
}
#endif
...
{
  return_value_if_fail(vg != NULL && vg->vt->set_stroke_vg_linear_gradient != NULL, RET_BAD_PARAMS);

  return vg->vt->set_stroke_vg_linear_gradient(vg, sx, sy, ex, ey, ramp_stops, ramp_stops_count);
}
