...
static dirty_rects_t* canvas_fix_rect_dirty_rects (const dirty_rects_t* r, dirty_rects_t* o) {
	if (r != NULL) 
	{
		canvas_fix_rect(&r->max, &o->max);
		int i;
		for (i = 0; i < r->nr; i++)
		{
			canvas_fix_rect(&r->rects[i], &o->rects[i]);
		}
		o->debug = r->debug;
		o->disable_multiple = r->disable_multiple;
		o->profile = r->profile;
		return o;
	}
	else {
...
ret_t canvas_set_clip_rect_dirty_rects(canvas_t* c, const dirty_rects_t* r_in) {
	wh_t lcd_w = 0;
	wh_t lcd_h = 0;
	dirty_rects_t r_fix;
	dirty_rects_t* r = canvas_fix_rect_dirty_rects(r_in, &r_fix);
	...
	if (c->lcd->set_clip_dirty_rects != NULL) {
		/* 在 opengl 的状态下让 vg 来处理裁剪区的大小直接交给 vg 来处理，去除 LCD 的大小限制的问题 */
		if (r) {
			//rect_t clip_r = rect_init(tk_max(0, r->x), tk_max(0, r->y), tk_max(0, r->w), tk_max(0, r->h));
			lcd_set_clip_dirty_rects(c->lcd, r_in);
		}
		else {
			rect_t clip_r = rect_init(0, 0, lcd_w, lcd_h);
			lcd_set_clip_rect(c->lcd, &clip_r);
		...
		if (r != NULL) {
			c->clip_left = tk_max(0, r->max.x);
			c->clip_top = tk_max(0, r->max.y);
			c->clip_right = tk_min(lcd_w - 1, r->max.x + r->max.w - 1);
			c->clip_bottom = tk_min(lcd_h - 1, r->max.y + r->max.h - 1);
		}
		else {
			c->clip_left = 0;
			c->clip_top = 0;
