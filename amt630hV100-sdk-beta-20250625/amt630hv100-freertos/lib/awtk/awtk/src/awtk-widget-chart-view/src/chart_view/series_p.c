...
// 计算距离的近似值
static float_t cal_length(float_t x, float_t y) {
  float n, f, r;
  int32_t i;
  n = x * x + y * y;
  f = n;
...
#define _VACANVAS_BEZIER_FIFO_ONE(i, index, size, fifo, d, dprev, ox, oy, draw_data_t)         \
  {                                                                                            \
    ...
    if (i == 0) {                                                                              \
      ...
      draw_data_t* dnext = (draw_data_t*)(series_fifo_get(fifo, i - 1));                       \
      float_t nextx, nexty, vx, vy, lprev, lnext, rnext, minx, maxx, miny, maxy;               \
      nextx = dnext->x;                                                                        \
      nexty = dnext->y;                                                                        \
      if (i == index) {                                                                        \
        if (vertical) {                                                                        \
          if (nextx - d->x != d->x - dprev->x) {                                               \
            nextx = d->x * 2 - dprev->x;                                                       \
          }                                                                                    \
        } else {                                                                               \
          if (nexty - d->y != d->y - dprev->y) {                                               \
            nexty = d->y * 2 - dprev->y;                                                       \
          }                                                                                    \
        }                                                                                      \
      }                                                                                        \
      lprev = cal_length(d->x - dprev->x, d->y - dprev->y);                                    \
      lnext = cal_length(d->x - nextx, d->y - nexty);                                          \
      lprev = tk_max(lprev, 0.001);                                                            \
      lnext = tk_max(lnext, 0.001);                                                            \
      rnext = lnext / (lnext + lprev);                                                         \
      vx = nextx - dprev->x;                                                                   \
      vy = nexty - dprev->y;                                                                   \
      cp2x = d->x - vx * (1 - rnext) * 0.5;                                                    \
      cp2y = d->y - vy * (1 - rnext) * 0.5;                                                    \
      nextcp1x = d->x + vx * rnext * 0.5;                                                      \
      nextcp1y = d->y + vy * rnext * 0.5;                                                      \
      minx = tk_min(nextx, d->x);                                                              \
      miny = tk_min(nexty, d->y);                                                              \
      maxx = tk_max(nextx, d->x);                                                              \
      maxy = tk_max(nexty, d->y);                                                              \
      nextcp1x = tk_clamp(nextcp1x, minx, maxx);                                               \
      nextcp1y = tk_clamp(nextcp1y, miny, maxy);                                               \
      vx = nextcp1x - d->x;                                                                    \
      vy = nextcp1y - d->y;                                                                    \
      cp2x = d->x - vx * lprev / lnext;                                                        \
      cp2y = d->y - vy * lprev / lnext;                                                        \
      minx = tk_min(dprev->x, d->x);                                                           \
      miny = tk_min(dprev->y, d->y);                                                           \
      maxx = tk_max(dprev->x, d->x);                                                           \
      maxy = tk_max(dprev->y, d->y);                                                           \
      cp2x = tk_clamp(cp2x, minx, maxx);                                                       \
      cp2y = tk_clamp(cp2y, miny, maxy);                                                       \
      vx = d->x - cp2x;                                                                        \
      vy = d->y - cp2y;                                                                        \
      nextcp1x = d->x + vx * lnext / lprev;                                                    \
      nextcp1y = d->y + vy * lnext / lprev;                                                    \
    }                                                                                          \
    _VACANVAS_BEZIER_TO(vg, ox + cp1x, oy + cp1y, ox + cp2x, oy + cp2y, ox + d->x, oy + d->y); \
  }
...
ret_t series_p_draw_smooth_line(widget_t* widget, vgcanvas_t* vg, style_t* style, float_t ox,
  ...
  if (color.rgba.a) {
    ...
    if (size == 2) {
      ...
      float_t nextcp1x = d->x;
      float_t nextcp1y = d->y;
      for (i = index + size - 2; i >= (int32_t)index; i--) {
        d = (series_data_draw_normal_t*)(series_fifo_get(fifo, i));
