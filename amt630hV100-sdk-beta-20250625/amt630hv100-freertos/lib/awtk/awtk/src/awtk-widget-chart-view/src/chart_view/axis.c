...
static ret_t axis_measure_series_offset(widget_t* widget) {
  float_t range;
  axis_t* axis = AXIS(widget);
  return_value_if_fail(axis != NULL, RET_BAD_PARAMS);

  range = axis_get_range(widget, TRUE);
  ...
  if (widget_is_series(iter)) {
    if (widget_get_prop_pointer(iter, SERIES_PROP_SERIES_AXIS_OBJ) == widget) {
      uint32_t offset = widget_get_prop_int(iter, SERIES_PROP_OFFSET, 0);
      uint32_t offset_max = widget_get_prop_int(iter, SERIES_PROP_OFFSET_MAX, 0);
      if (offset > offset_max) {
        widget_set_prop_int(iter, SERIES_PROP_OFFSET, offset_max);
      }
    }
  }
...
ret_t axis_set_range(widget_t* widget, float_t min, float_t max) {
  ...
  axis_measure_series_offset(widget);
