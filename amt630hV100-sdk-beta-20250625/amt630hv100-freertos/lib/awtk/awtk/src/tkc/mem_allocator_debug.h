...
static int item_sort(const void *a, const void *b) {
  ...
  m1 = (mem_entry_t *)a;
  m2 = (mem_entry_t *)b;
  return (int)m2->size - (int)m1->size;
}

#define DEBUG_LOG_SIZE   0// 8192*16   // 大于该值的内存分配将被打印
static inline void mem_entry_dump(void) {
  ...
  uint32_t geq1024000 = 0;
  uint32_t used_block = 0;
  uint32_t used_size = 0;

  uint32_t n = ARRAY_SIZE(s_mem_log);
  // 20220103 排序
  qsort((void *)s_mem_log, n, sizeof(mem_entry_t), item_sort);
  ...
  log_info(
      ...
      ">=10240=%u >=102400=%u >=1024000=%u\n",
      used_block, used_size, leq8, leq16, leq32, leq48, leq64, geq1024, geq10240, geq102400, geq1024000);
