...
#ifdef WITH_VG_GPU
	// widget->w / 2 --> widget->w / 2 - (r->w / 2 + 1)
	// widget->w / 2 --> widget->w / 2 + (r->w / 2 + 1)
	bitmap_t *bmp = bitmap_create_ex((n-1)*2 + 1, widget->h, ((n - 1) * 2 + 1) * 4, BITMAP_FMT_RGBA8888);
	if (bmp)
	{
		uint8_t* img_data = bitmap_lock_buffer_for_write(bmp);
		if (img_data)
		{
			uint32_t *img_data_32_1, *img_data_32_2;
			uint8_t *tmp_1, *tmp_2;
			img_data_32_1 = ((uint32_t *)img_data) + n;
			img_data_32_2 = ((uint32_t *)img_data) + n;
			for (i = 0; i < n; i++)
			{
				mask_color.rgba.a = 0xff * easing((float_t)i / (float_t)n);
				tmp_1 = (uint8_t *)img_data_32_1;
				tmp_2 = (uint8_t *)img_data_32_2;
				for (int j = 0; j < widget->h; j++)
				{
					*(uint32_t *)tmp_1 = mask_color.color;
					tmp_1 += bmp->line_length;
					*(uint32_t *)tmp_2 = mask_color.color;
					tmp_2 += bmp->line_length;
				}
				img_data_32_1 --;
				img_data_32_2 ++;
			}
			bitmap_unlock_buffer(bmp);
			rect_t src = { 0, 0, (n - 1) * 2 + 1, widget->h };
			rect_t dst = { (widget->w - ((n - 1) * 2 + 1))/2, 0, (n - 1) * 2 + 1, widget->h };
