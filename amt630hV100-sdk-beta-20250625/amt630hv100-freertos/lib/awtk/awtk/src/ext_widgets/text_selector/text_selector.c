...
#ifdef WITH_VG_GPU
	  bitmap_t *bmp = bitmap_create_ex(widget->w, n*2, widget->w * 4, BITMAP_FMT_RGBA8888);
	  if (bmp)
	  {
		  uint8_t* img_data = bitmap_lock_buffer_for_write(bmp);
		  if (img_data)
		  {
			  // n - 1, 0
			  uint8_t *img_data_32_1, *img_data_32_2;
			  uint32_t *tmp_1, *tmp_2;
			  img_data_32_1 = (img_data) + bmp->line_length * (n - 1);
			  img_data_32_2 = (img_data) + bmp->line_length * (n );
			  for (i = 0; i < n; i++)
			  {
				  mask_color.rgba.a = (uint8_t)(0xff * easing((float_t)i / (float_t)n));
				  
				  tmp_1 = (uint32_t *)img_data_32_1;
				  tmp_2 = (uint32_t *)img_data_32_2;
				  for (int j = 0; j < widget->w; j++)
				  {
					  *tmp_1++ = mask_color.color;
					  *tmp_2++ = mask_color.color;
				  }
				  img_data_32_1 -= bmp->line_length;
				  img_data_32_2 += bmp->line_length;
			  }
			  bitmap_unlock_buffer(bmp);
			  rect_t src = { 0, 0, widget->w, 2 * n };
			  rect_t dst = { 0, 0, widget->w, 2 * n };
			  canvas_draw_image(c, bmp, &src, &dst);
		  }
