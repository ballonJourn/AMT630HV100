...
uint64_t tk_thread_self(void) {
  ...
  uint64_t uint64_self = 0;
  if (sizeof(pthread_t) != 8 && sizeof(pthread_t) != 4)
  {
    log_error("%s:%d sizeof(pthread_t)(%d) != 8 || sizeof(pthread_t)(%d) != 4 failed!\n", __FUNCTION__, __LINE__, sizeof(pthread_t), sizeof(pthread_t));
  }
  else if (sizeof(pthread_t) == 8)
  {
    uint64_self = *(uint64_t *)(&thread);
  }
  else if (sizeof(pthread_t) == 4)
  {
    uint64_self = *(uint32_t *)(&thread);
  }
  return uint64_self;
