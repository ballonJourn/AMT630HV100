...
#define	NVG_MAX_PATH_COORDS	512
#define NVG_MAX_PATH_SEGMENTS 512
#define NVG_MAX_DIRTY_RECTS 32	// VG最大支持32个裁剪区
struct NVGcontext {
	NVGparams params;
	float* commands;
	int ccommands;
...
static void ProcessDirtyRectangle(NVGcontext* ctx);
static void nvg_set_alpha(NVGcontext* ctx, NVGstate* state, float alpha);
static int nvgTestRectangleIntersection(float x1, float y1, float w1, float h1, float x2, float y2, float w2, float h2);
...
void nvgReset(NVGcontext* ctx)
{
	...
	state->alpha = 1.0f;
	//nvg_set_alpha(ctx, state, state->alpha);
...
static float global_alpha = 2.0;
static NVGscissor global_scissor = { 0 };
static void nvg_set_alpha(NVGcontext* ctx, NVGstate* state, float alpha)
{
...
// NANOVG矩阵为2行3列,存储按行组织. { sx, shy, shx, sy, tx, ty }
// OPENVG矩阵为3行3列,存储按行组织. { sx, shy, w0, shx, sy, w1, tx, ty, w2 }
static void nanovg_matrix_to_openvg_matrix(float nvg_matrix[6], float vg_matrix[9])
{
	vg_matrix[0] = nvg_matrix[0];
	vg_matrix[1] = nvg_matrix[1];
	vg_matrix[2] = 0;
	vg_matrix[3] = nvg_matrix[2];
	vg_matrix[4] = nvg_matrix[3];
	vg_matrix[5] = 0;
	vg_matrix[6] = nvg_matrix[4];
	vg_matrix[7] = nvg_matrix[5];
	vg_matrix[8] = 1.0;
}

static void openvg_matrix_to_nanovg_matrix(float vg_matrix[9], float nvg_matrix[6])
{
	nvg_matrix[0] = vg_matrix[0];
	nvg_matrix[1] = vg_matrix[1];
	nvg_matrix[2] = vg_matrix[3];
	nvg_matrix[3] = vg_matrix[4];
	nvg_matrix[4] = vg_matrix[6];
	nvg_matrix[5] = vg_matrix[7];
}
...
void nvgUpdateImage(NVGcontext* ctx, int image, const unsigned char* data, int image_linelen)
{
	int w, h;
								...
								int image, float alpha)
{
	...
	//if (alpha != 1.0)
	//	printf("nvgImagePattern alpha=%f\n", alpha);
...
// 设置脏矩形
void nvgScissors(NVGcontext* ctx, int* dirty_rect, int dirty_size, int *max)
{

	//printf("nvgScissors %d\n", dirty_size);
#ifdef DIRTY_RECTS_CLIP_SUPPORT
	NVGstate* state = nvg__getState(ctx);

	if (dirty_size > NVG_MAX_DIRTY_RECTS)
	{
		printf("illegal dirty_size(%d). Maximum 32 dirty rectangles supported,\n", dirty_size);
		// 将脏矩形置为空, 禁止脏矩形处理
		dirty_size = 0;
	}
	if ( dirty_size == 0 || dirty_rect == NULL)
	{
		// 将脏矩形置为最大尺寸
		ctx->cb_dirty_rect = 1;
		ctx->dirty_rect[0] = 0; // x
		ctx->dirty_rect[1] = 0; // y
		ctx->dirty_rect[2] = ctx->w;	// w
		ctx->dirty_rect[3] = ctx->h; // h

		state->scissor.x = ctx->dirty_rect[0];
		state->scissor.y = ctx->dirty_rect[1];
		state->scissor.w = ctx->dirty_rect[2];
		state->scissor.h = ctx->dirty_rect[3];
	}
	else
	{
		// 20211008 采用最大脏矩形, AWTK指针demo刷新正常. 另外, 采用最大脏矩形时"洁净新风系统"DEMO刷新时间比较下面"多个脏矩形区域"方案更短.
		nvgTransformIdentity(state->scissor.xform);
		state->scissor.xform[4] = max[0];
		state->scissor.xform[5] = max[1];
		nvgTransformMultiply(state->scissor.xform, state->xform);
		ctx->dirty_rect[4 * 0 + 0] = state->scissor.xform[4]; // x
		ctx->dirty_rect[4 * 0 + 1] = state->scissor.xform[5]; // y
		ctx->dirty_rect[4 * 0 + 2] = max[2];	// w
		ctx->dirty_rect[4 * 0 + 3] = max[3]; // h
		ctx->cb_dirty_rect = 1;
		...
		//state->scissor.x = max[0];
		//state->scissor.y = max[1];
		//state->scissor.w = max[2];
		//state->scissor.h = max[3];

	}
	vgSetiv(VG_SCISSOR_RECTS, ctx->cb_dirty_rect * 4, ctx->dirty_rect);
	vgSeti(VG_SCISSORING, VG_TRUE);
	//ProcessDirtyRectangle(ctx);
	//nvg_set_alpha(ctx, state, state->alpha);
