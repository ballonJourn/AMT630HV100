...
struct NVGpaint {
	...
	VGImage	alphaImage;
	VGfloat alpha; 
  VGfloat rampStops[10 * 5];    // 20211228 增加OpenVG风格的多分段线性渐变填充, 支持最多10段
  int rampStopsCount;
	union {
		VGfloat linearGradient[4];
		VGfloat radialGradient[5];
		VGfloat pattern[3];	// pattern的缩放系数x,y,angle
	};
...
#if defined(WITH_VG_GPU)
// 20211218 增加VG风格的线性渐变填充
// rampStops VG格式的rampStops描述, 如下为一个实例
//   static VGfloat rampStops_innerPath[30] = {
//  offset    R        G       B     Alpha
//  0.0000f, 0.898f, 0.263f, 0.133f, 1.0f,		//<stop  offset="0.0000" style="stop-color:#E54322"/>
...
//  1.0000f, 0.247f, 0.000f, 0.031f, 1.0f,		//<stop  offset="1.0000" style="stop-color:#3F0008"/>
// };
// rampStopsCount rampStops浮点数组的个数
NVGpaint nvgLinearGradientEx(NVGcontext* ctx,
  float sx, float sy, float ex, float ey,
  float *rampStops, int rampStopsCount);
#endif
...
int nvgFastClear(NVGcontext* ctx, float x, float y, float w,
	float h, NVGcolor color);

void nvgDrawImage(NVGcontext* ctx, int image, int x, int y, int w, int h);

#ifdef WITH_VG_GPU
// ImagePattern扩展函数, 支持tiling mode
NVGpaint nvgImagePatternRepeat(NVGcontext* ctx,
