...
static ret_t guage_second_pointer_on_destroy(widget_t* widget) {
...
static ret_t guage_second_pointer_get_prop(widget_t* widget, const char* name, value_t* v) {
  (void)name;
  (void)v;
  return_value_if_fail(widget != NULL, RET_BAD_PARAMS);
...
static ret_t guage_second_pointer_set_prop(widget_t* widget, const char* name, const value_t* v) {
  return_value_if_fail(widget != NULL, RET_BAD_PARAMS);
  if (tk_str_eq(name, WIDGET_PROP_IMAGE)) {
    return guage_second_pointer_set_image(widget, value_str(v));
  }

  return RET_NOT_FOUND;
}

static const char* s_guage_second_pointer_properties[] = {NULL};

static const widget_vtable_t s_guage_second_pointer_vtable = {
    .size = sizeof(guage_second_pointer_t),
    .type = WIDGET_TYPE_GUAGE_SECOND_POINTER,
    .clone_properties = s_guage_second_pointer_properties,
    .persistent_properties = s_guage_second_pointer_properties,
    .create = guage_second_pointer_create,
    .on_paint_self = guage_second_pointer_on_paint_self,
    .on_destroy = guage_second_pointer_on_destroy,
    .get_prop = guage_second_pointer_get_prop,
    .set_prop = guage_second_pointer_set_prop};
...
widget_t* guage_second_pointer_create(widget_t* parent, xy_t x, xy_t y, wh_t w, wh_t h) {
  ...
  widget_t* widget = widget_create(parent, &s_guage_second_pointer_vtable, x, y, w, h);
