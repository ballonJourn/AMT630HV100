...
static ret_t on_slidemenu_visit(void* ctx, const void* data) {
...
static ret_t on_dial_long_pressed(void* ctx, event_t* e) {
  ...
  if (win != NULL) {
    ...
    widget_t* slidemenu = widget_lookup(win, "select:slidemenu", TRUE);
    if (slidemenu != NULL) {
      widget_foreach(slidemenu, on_slidemenu_visit, win);
      slide_menu_set_value(slidemenu, global_data.watch_type);
    } else {
      assert(FALSE);
    }
  }
