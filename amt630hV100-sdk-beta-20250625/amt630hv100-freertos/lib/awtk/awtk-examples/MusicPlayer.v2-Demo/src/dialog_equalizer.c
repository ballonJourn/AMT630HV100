...
static ret_t on_slider_changing(void* ctx, event_t* e) {
  ...
  widget_t* chart_view = widget_lookup(win, "chart_view", TRUE);
  if (chart_view) {
...
ret_t open_equalizer_dialog() {
  widget_t* dialog = dialog_open("equalizer/equalizer");
  widget_t* wm = window_manager();
  ...
  if (dialog) {
    ...
    widget_t* chart_view = widget_lookup(dialog, "chart_view", TRUE);
    if (chart_view) {
      set_series_data(val_slider, chart_view, SLIDER_COUNT);
    }

    return dialog_modal(dialog);
    // dialog_create(dialog, 800, 480, 400, 355);
    // return RET_OK;
  }
