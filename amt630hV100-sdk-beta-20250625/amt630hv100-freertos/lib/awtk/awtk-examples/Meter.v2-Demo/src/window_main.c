...
extern ret_t qr_set_value(widget_t* widget, const char* value);
#ifdef CARLINK_ENABLE
extern int get_qr_text_buf(char *buf, int len);
#endif
extern char test_key_value;
widget_t* view_qr = NULL;
widget_t* qr_ec = NULL;

static ret_t onShowQrCode(const timer_info_t* t) {
	if (view_qr == NULL) {
		view_qr = widget_child(WIDGET(t->ctx), "view_qr");
	}
	/* if(qr_ec == NULL){
		qr_ec = widget_child(WIDGET(t->ctx), "qr_ec");
	}*/
	//printf("strQrText:%s +++++\r\n", strQrText);
#ifdef CARLINK_ENABLE
	...
	if (strQrText[0] != '\0') { 
		...
		memset(strQrText,sizeof(strQrText),0);
	}
#endif
	if (test_key_value == 1) {
		widget_set_visible(view_qr,1);
	} else {
		widget_set_visible(view_qr,0);
	}
...
static int32_t get_random_int(int32_t min_num, int32_t max_num) {
  return (int32_t)fmod(rand(), max_num - min_num) + min_num;
}
...
ret_t application_init() {
  ...
  widget_set_style_str(win,"normal:bg_image_draw_type","default");
  if (win) {
    create_images(win);
    qr_ec = widget_lookup(win, "qr_ec", TRUE);
    timer_add(on_update_led_status, win, 1000);
	widget_add_timer(win, onShowQrCode, 500);
  }
