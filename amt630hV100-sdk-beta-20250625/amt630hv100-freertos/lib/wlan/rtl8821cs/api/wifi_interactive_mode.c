...
static void cmd_wifi_iwpriv(int argc, char **argv)
{
	if(argc == 2 && argv[1]) {
		...
		if (argc == 3) {
			char cmd_buf[32] = {0};
			sprintf(cmd_buf, "%s %s", argv[1], argv[2]);
			wext_private_command(WLAN0_NAME, cmd_buf, 1);
		} else
...
static void cmd_rawdata_sendData(int argc, char **argv)
{
	...
	wext_send_rawdata(WLAN0_NAME,(const unsigned char*)Frame_buf, sizeof(Frame_buf));
}
...
void cmd_test(const char* temp_uart_buf)
{
	int i, argc;
	char *argv[MAX_ARGC];
	//char temp_uart_buf[100] = "wifi_info";

	if((argc = parse_cmd((char *)temp_uart_buf, argv)) > 0) {
		int found = 0;
