...
#elif LV_USE_GPU_ARK_BLEND2D
	#include "../lv_gpu/lv_gpu_ark_blend2d.h"
#endif
...
#elif LV_USE_GPU_ARK_BLEND2D
            if(lv_area_get_size(draw_area) >= LV_GPU_ARK_BLEND2D_FILL_SIZE_LIMIT) {
				int32_t disp_h = lv_area_get_height(disp_area); 
                if (lv_gpu_ark_blend2d_fill(disp_buf, disp_w, disp_h, draw_area, color, opa) == LV_RES_OK) {
                	return;
                }
            }			
#endif
...
#elif LV_USE_GPU_ARK_BLEND2D
            if(lv_area_get_size(draw_area) >= LV_GPU_ARK_BLEND2D_BLIT_SIZE_LIMIT) {

                lv_gpu_ark_blend2d_blit_info_t blit;
                ...
                blit.opa = opa;
				blit.alpha_byte = false;

                if(lv_gpu_ark_blend2d_blit(&blit) == LV_RES_OK) {
                    return;
                }
                /* Fall down to SW render in case of error */
            }			
