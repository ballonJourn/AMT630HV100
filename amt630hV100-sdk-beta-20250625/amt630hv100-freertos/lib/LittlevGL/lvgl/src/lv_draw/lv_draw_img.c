...
#elif LV_USE_GPU_ARK_BLEND2D && LV_COLOR_DEPTH == 32
            if(lv_area_get_size(&draw_area) >= LV_GPU_ARK_BLEND2D_BLIT_OPA_SIZE_LIMIT) {
				lv_color_t * disp_buf = vdb->buf_act;
                lv_gpu_ark_blend2d_blit_info_t blit;

                blit.src = (lv_color_t*)map_p;
