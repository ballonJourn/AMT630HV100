...
void lv_img_set_src(lv_obj_t * img, const void * src_img)
{
	...
	/* Do nothing if the src_img is not changed */
	if (src_type == ext->src_type) {
		if (src_type == LV_IMG_SRC_FILE && ext->src && !strcmp(ext->src, src_img))
			return;
		/* else if (src_type == LV_IMG_SRC_VARIABLE && ext->src == src_img)
			return;*/
	}
